...
fn parse_input(input: &String) -> HashMap<(&str, &str), i32> {
    Regex::new(r"(?m)(\w+) would (gain|lose) (\d+) .*? to (\w+)")
        .unwrap()
        ...
        .map(|capture| {
            ...
            let change_in_happiness =
                sign * capture.get(3).unwrap().as_str().parse::<i32>().unwrap();
...
fn permutation_happiness(perm: Vec<&&str>, relations: &HashMap<(&str, &str), i32>) -> i32 {
    perm.iter()
        ...
        .map(|(l, c, r)| relations.get(&(**c, **l)).unwrap() + relations.get(&(**c, **r)).unwrap())
        .sum()
}
