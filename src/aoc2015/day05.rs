...
pub fn solve_part_one(input: &String) -> i32 {
    input.lines().filter(is_nice_string_one).count() as i32
}
...
fn is_nice_string_one(s: &&str) -> bool {
    ...
    let fulfils_vowels_condition = s
        .chars()
        .filter(|c| match c {
            'a' | 'e' | 'i' | 'o' | 'u' => true,
            _ => false,
    ...
    let fulfils_contiguous_condition = s.chars().enumerate().any(|(i, c)| {
    ...
    let fulfills_exclusion_condition = ["ab", "cd", "pq", "xy"]
        .iter()
...
pub fn solve_part_two(input: &String) -> i32 {
    input.lines().filter(is_nice_string_two).count() as i32
}
...
fn is_nice_string_two(s: &&str) -> bool {
    ...
    let fulfils_pair_condition = pairs.iter().enumerate().any(|(i, c)| {
        pairs
            .iter()
            .enumerate()
    ...
    let fulfils_repetition_condition = chars
        .iter()
        .enumerate()
