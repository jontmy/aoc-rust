...
pub fn solve_part_one(input: &String) -> i32 {
    let (gamma, epsilon): (Vec<char>, Vec<char>) = frequencies(input)
        .iter()
        .map(|(zeroes, ones)| {
            if zeroes > ones {
                ('0', '1')
...
fn frequencies(bins: &String) -> Vec<(usize, usize)> {
    ...
    (0..index_char_map.len())
        .map(|i| {
            let (zeroes, ones): (Vec<char>, Vec<char>) = index_char_map
                .get(&i)
                .unwrap()
                .iter()
                .partition(|&&c| c == '0');
...
pub fn solve_part_two(input: &String) -> i32 {
    ...
    let initial = (lines.iter().collect_vec(), lines.iter().collect_vec());

    let (o2, co2) = (0..len).into_iter().fold(initial, |(o2, co2), i| {
...
fn bit_criteria(o2: &Vec<&String>, co2: &Vec<&String>, index: usize) -> (String, String) {
    let (zeroes, ones): (Vec<char>, Vec<char>) = o2
        .iter()
        .map(|bin| bin.chars().nth(index).unwrap())
        .partition(|&c| c == '0');

    let most_common = if ones.len() >= zeroes.len() { '1' } else { '0' };

    let (zeroes, ones): (Vec<char>, Vec<char>) = co2
        .iter()
        .map(|bin| bin.chars().nth(index).unwrap())
        .partition(|&c| c == '0');
